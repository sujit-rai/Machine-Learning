...
class pruning{
	static String rules;
	static int pruned_nodes;
	static Vector<ptree> post_node = new Vector<ptree>();
	public static void main(String args[]){
		Scanner sc = new Scanner(System.in);
		String rulefile = System.getProperty("user.dir")+"/rules.txt";
		String featfile = System.getProperty("user.dir")+"/aclImdb/train/labeledBow.feat";
		...
		// TEST
		for(int s=0;s<post_node.size()+1;s++){
			int fulltest = 0;
			if(s==post_node.size()){
				System.out.println("Test complete data on test case ?");
				sc.nextLine();
				featfile = System.getProperty("user.dir")+"/aclImdb/test/labeledBow.feat";
				fulltest = 1;
			}
		double newaccuracy=0.0,oldaccuracy = 0.0,fullfrequency = 0.0;
		for(int u=0;u<2;u++){
			
			if(u==1&&fulltest==1)
				break;
			if(u==1)
				post_node.get(s).prune = 1;
		...
		try(BufferedReader br = new BufferedReader(new FileReader(featfile))){
			...
			while((line=br.readLine())!=null){
				line_counter++;
				if(line_counter<12000&&fulltest==0){
					//System.out.println("inside if");
					
					continue;
				}

				if(line_counter==13000&&fulltest==0)
					break;
				...
				while(temp!=null){
					...
					else if(arr.contains(temp.attr)){
					...
					if(u == 0)
						oldaccuracy = (double)correct/(double)total;
					else
						newaccuracy = (double)correct/(double)total;
					if(fulltest == 1)
						fullfrequency = oldaccuracy;
				}
			}
			...
			if(fulltest == 1){
				System.out.println("accuracy after pruning : "+fullfrequency);

				break;
			}
			if(newaccuracy>=oldaccuracy){
				System.out.println("node pruned : "+post_node.get(s).attr);
				pruned_nodes ++ ;
			}
			else{
				post_node.get(s).prune = 0;
				System.out.println("inside else");
			}
			System.out.println("newaccuracy : "+newaccuracy);
			System.out.println("oldaccuracy : "+oldaccuracy);
			//System.out.println("one node checked, continue ?");
		...
		temp = root;
		System.out.println("number of nodes : "+countnodes(temp));
		System.out.println("number of pruned nodes : "+pruned_nodes);

		
	}

	public static void formrules(ptree node){
	...
	public static int countnodes(ptree root){
		if(root == null || root.prune == 1)
			return 0;
	...
	public static void postorder(ptree root){
		if(root == null)
			return;
		postorder(root.left);
