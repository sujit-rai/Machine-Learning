...
class Detree{
	...
	public static void main(String arg[]){
		...
		String tablefile = System.getProperty("user.dir")+"/tableindexes.txt";
		PrintWriter writer = null;
		q2 = (args.length>0)?Integer.parseInt(args[0]):0;
		q3 = (args.length>0)?Double.parseDouble(args[0]):0;
		System.out.println(q3);
		System.out.println(1000/q3);
		if(q2 == 2)
			q21 = Integer.parseInt(args[1]);
		...
		try(BufferedReader br = new BufferedReader(new FileReader(tablefile))){
			...
			int noise_counter = (int) (1000*Double.parseDouble(args[1]));
		...
		for(int i=0;i<arr.size();i++)
			root.attributes_left.add(arr.get(i));
		for(int i=0;i<table.size();i++)
			root.indexes.add(i);

		ID3(root);
		System.out.println("Number of Terminal Nodes : "+terminal_nodes);
		System.out.println("decision dtree is built completely");
		System.out.println("store decision dtree ?");
		Scanner sc = new Scanner(System.in);
		sc.nextLine();
		String rules = formrules(root,"");
		try {
			writer = new PrintWriter("rules.txt","UTF-8");
			writer.println(rules);
			writer.close();
		} catch (IOException e){}
	}

	public static String formrules(dtree node,String rules){
		if(node.left == null)
	...
	public static int splitattribute(dtree node){
		return 0;
	}

	public static void ID3(dtree node){
		...
		if(node.attributes_left.size()==0 || (q2==2&&(q21 == 1)&&(node.probability > Double.parseDouble(args[2]) || node.probability < 1-Double.parseDouble(args[2])))
											||(q2==2&&(q21 == 2)&&(node.depth>10))||(q2==2&&(q21==3)&&(table_pos + table_neg)<10)||(q2==2&&(q21==4)&&(table_pos + table_neg)<20)){
			terminal_nodes++;
