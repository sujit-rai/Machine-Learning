...
def NewtonRaphson(X,Y,W,lamda,iter):
	for j in range(iter):
		...
		b=Misclassify(X,Y,W)
		print 'Iteration number :',j,' Examples Misclassified:',b
		if b==0:
			break
	return Misclassify(X,Y,W),W
...
Y=np.array(Y)
W=np.array(W)
M,W=GradientDescent(X,Y,W,alpha,lamda,5000000)
...
# for i in range(W.shape[0]):
#  	W[i][0]=0.0004
#Newton Raphson Degree 2 W=-0.02
#Newton Raphson Degree 3 W=-0.004
print '****************************Initial W*************************************'
print W
fig,ax=plt.subplots()
...
plt.savefig('q459output.png')
plt.show(block=False)
plt.pause(3)
plt.close()
for i in range(W.shape[0]):
	W[i][0]=-0.0002
#Newton Raphson W=0.0004 lamda=0.001 for degree 2,3
lamda=0.1
...
plt.ylabel('Feature x2----->')
legend = ax.legend(loc='upper right')
titl='Newton Raphson lamda '+str(lamda)
plt.title(titl)
...
for i in range(Z.shape[0]):
	for j in range(Z.shape[1]):
		Z[i][j] = predictVal(x1vals[j],x2vals[i],W);
plt.contour(X1,X2,Z,levels=np.linspace(0.499999,0.5,2))
plt.savefig('q111110output.png')
plt.show(block=False)
plt.pause(3)
plt.close()
f.close()